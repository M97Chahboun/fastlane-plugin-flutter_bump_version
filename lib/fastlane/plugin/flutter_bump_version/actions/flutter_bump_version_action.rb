  ...
  module Actions
    ...
    class FlutterBumpVersion
      ...
      def bump_version(part)
        ...
        patch = build_exist ? split_current_version[2].split("+")[0] : split_current_version[2]
        build = build_exist ? split_current_version[2].split("+")[1] : "0"
        ...
        current_version_in_hash['major'] = is_major ? current_version_in_hash['major'] + 1 : current_version_in_hash['major']
        current_version_in_hash['minor'] = if is_minor
                                             current_version_in_hash['minor'] + 1
                                           else
                                             is_major ? 0 : current_version_in_hash['minor']
                                           end
        current_version_in_hash['patch'] = if is_patch
                                             current_version_in_hash['patch'] + 1
                                           else
                                             is_major || is_minor ? 0 : current_version_in_hash['patch']
                                           end
        current_version_in_hash['build'] = if is_build
